...
// Free transpose convolution layer
...
void initialize_decoder_weights(Decoder* decoder) {
    ...
    CUDA_CHECK(cudaMemcpy(decoder->conv1->d_weights, h_conv1_weights, conv1_weight_size * sizeof(float), cudaMemcpyHostToDevice));
    free(h_conv1_weights);
    ...
    CUDA_CHECK(cudaMemcpy(decoder->conv2->d_weights, h_conv2_weights, conv2_weight_size * sizeof(float), cudaMemcpyHostToDevice));
    free(h_conv2_weights);
    ...
    CUDA_CHECK(cudaMemcpy(decoder->conv3->d_weights, h_conv3_weights, conv3_weight_size * sizeof(float), cudaMemcpyHostToDevice));
