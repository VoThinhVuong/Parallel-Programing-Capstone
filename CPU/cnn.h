...
// Decoder network
typedef struct {
    ConvLayer* conv1;              // 128ch → 64ch, 8×8
    UpsampleLayer* upsample1;      // 8×8 → 16×16
    ConvLayer* conv2;              // 64ch → 64ch, 16×16
    UpsampleLayer* upsample2;      // 16×16 → 32×32
    ConvLayer* conv3;              // 64ch → 3ch, 32×32
    
    float* conv1_relu;             // After Conv1 + ReLU
    float* conv1_relu_grad;        // Gradient
    float* conv2_relu;             // After Conv2 + ReLU
    float* conv2_relu_grad;        // Gradient
