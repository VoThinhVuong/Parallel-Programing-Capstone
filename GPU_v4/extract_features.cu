...
int main(int argc, char** argv) {
...
#ifdef _WIN32
    system("if not exist ..\\extracted_features mkdir ..\\extracted_features");
#else
    system("mkdir -p ../extracted_features");
#endif
    ...
    if (f) {
        ...
        if (fread(&n, sizeof(int), 1, f) == 1) {
            ...
            if (fread(train_labels, sizeof(uint8_t), n, f) != (size_t)n) {
                fprintf(stderr, "Warning: Failed to read all train labels\n");
            }
        }
        fclose(f);
    }
    ...
    if (f) {
        ...
        if (fread(&n, sizeof(int), 1, f) == 1) {
            ...
            if (fread(test_labels, sizeof(uint8_t), n, f) != (size_t)n) {
                fprintf(stderr, "Warning: Failed to read all test labels\n");
            }
        }
        fclose(f);
    }
