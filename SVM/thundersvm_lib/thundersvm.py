...
# if path.exists("./thundersvm.dll"):
#     thundersvm = CDLL("./thundersvm.dll")
# else:
...
if platform == "win32" and sys.version_info >= (3, 8):
    ...
    for cuda_path in cuda_paths:
        if path.exists(cuda_path):
            os.add_dll_directory(cuda_path)
            print(f"Added CUDA DLL directory: {cuda_path}")
    
    # Add thundersvm bin directory itself
    dll_dir = path.abspath("./")
...
try:
    ...
    print("ThunderSVM DLL loaded successfully!")
except Exception as e:
    print(f"Error loading ThunderSVM DLL: {e}")
    raise FileNotFoundError("Please build the library first!", e)
